...
class KaTimbangSync(models.Model):
    ...
    def _do_sync(self, date_from=None, date_to=None, cron_mode=False):
        ...
        # Cache register & mbs untuk menghindari query berulang
        register_cache = {}
        petani_cache = {}
        mbs_cache = {}
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            # Resolve register_id (dengan cache)
            kode_register = row['register'] or ''
            if kode_register not in register_cache:
                reg = Register.search([('kode_register', '=', kode_register)], limit=1)
                register_cache[kode_register] = reg.id if reg else False
                petani_cache[kode_register] = reg.petani_id.id if reg and reg.petani_id else False
            register_id = register_cache[kode_register]
            petani_id   = petani_cache[kode_register]

            # Resolve mbs_id (dengan cache)
            ...
            mbs_id = mbs_cache[mbs_kode]

            # Hitung weight_kw dan bobot_tebu
            ...
            existing = Tebu.search([('sync_key', '=', sync_key)], limit=1)
            if existing:
                existing.write(vals)
                count_update += 1
                action = 'UPDATE'
            ...
            if idx % 100 == 0 or idx == total_rows:
                ...
                _logger.debug(
                    '[KA-TIMBANG] [%s] sync_key: %s | date_out: %s | netto: %.2f kg',
                    action, sync_key, row['date_out'], float(row['weight_net'] or 0)
                )
